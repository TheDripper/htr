...
<div id=frame>
	...
	<div class="slide" v-for="slide in $store.state.slides" :id="slide.id" :data-slide="slide.img" :style="{ backgroundImage: 'linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)),url(' +slide.img+ ')' }">
		<h1>{{ slide.text }}</h1>
		<h4 class=name>{{ slide.name }}</h4>
		<p>{{ slide.copy }}</p>
		<img src=/trace.png id=trace />
		<img src=/tracerow.png id=tracerow />
