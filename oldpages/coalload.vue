...
<div id=frame>
	...
	<div class="slide" v-for="slide in slides" :id="slide.id" :data-slide="slide.img" :style="{ backgroundImage: 'linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)),url(' +slide.img+ ')' }">
		...
		<h1>{{ slide.text }}</h1>
		<p>{{ slide.copy }}</p>
		<a v-if="slide.butt" class="bindme opener" @click='vert'>{{ slide.name }}<img src=~/assets/down.png /></a>
		<div class=subs>
	...
	asyncData({store}) {
		...
		return {
			slides: [
				{
					img: '/coalition.png',
					text: 'We Must Face Climate Change Together',
					id: 'coalition',
					name: "The Coalition",
					...
					subs: [
						{
							img: '/coalition_who.png',
							text: 'Our Coalition',
							copy: "Haiti Takes Root (HTR) is a new approach to Haiti’s environmental crisis.  Instead of creating a new organization, HTR is an initiative to strengthen capacity and coordination among existing stakeholders in order to manage Haiti’s natural resources and adapt to the impacts of climate change. In order to break out of the pattern of project cycles and isolated, individual efforts, HTR unites actors to embrace approaches grounded in lessons learned over decades, in Haiti and beyond.",
							name: 'Haiti Takes Root'
						}
					]
